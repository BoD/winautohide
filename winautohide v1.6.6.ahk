 ...
 */
menuAbout:
	MsgBox, 8256, About, BoD winautohide v1.6.6 Mod By WT.`n`nThis program and its source are in the public domain.`nContact BoD@JRAF.org for more information.
return
	...
	if (autohide_%winId%) { ; window is on the list of 'autohide' windows
		...
		if (needHide) {
			...
			if (mode = "right") {
				...
				hidden_%needHide%_y := S%MonitorIndex%MonitorTop - new_height + 1
			} else { ; down
				showing_%needHide%_x := new_%needHide%_x
				showing_%needHide%_y := S%MonitorIndex%MonitorBottom - new_height

				hidden_%needHide%_x := new_%needHide%_x
				hidden_%needHide%_y := S%MonitorIndex%MonitorBottom - 2
			}
			
			for j, ele in Array {
				if(Array[j,1] == %needHide%){
					Array[j,2] :=showing_%needHide%_x
					Array[j,3] :=showing_%needHide%_y
					Array[j,4] :=hidden_%needHide%_x
					Array[j,5] :=hidden_%needHide%_y
					Array[j,6] :=new_width
					Array[j,7] :=new_height
				}
			}
			
			tempa := new_%needHide%_x
			tempb := new_%needHide%_y
			
			MouseGetPos, xpos, ypos
			;MsgBox "new_%needHide%_x: " %tempa% `n "new_%needHide%_y: " %tempb% `n "xpos: " %xpos% `n "ypos: " %ypos%
			if((xpos < -20) OR (xpos > (new_width + 20)) OR (ypos < -20) OR (ypos > (new_height + 20))){
	...
	if (autohide_%curWinId%) {
		...
		for j, ele in Array {
			if(Array[j,1] == %curWinId%){
				Array[j,2] :=showing_%curWinId%_x
				Array[j,3] :=showing_%curWinId%_y
				Array[j,4] :=hidden_%curWinId%_x
				Array[j,5] :=hidden_%curWinId%_y
				Array[j,6] :=orig_width
				Array[j,7] :=orig_height
			}
			else {
				Array.Insert([%curWinId%,showing_%curWinId%_x,showing_%curWinId%_y,hidden_%curWinId%_x,hidden_%curWinId%_y,orig_width,orig_height])
			}
		}
